    ...
    public static ArrayList<Terremoto> extractTerremotos(JSONObject response) {
        ...
        try {
            ...
            for (int i = 0 ; i < terremotoArray.length(); i++){
                ...
                double magnitud = terremotoItem.getDouble(KEY_MAGNITUD);
                String lugar    = terremotoItem.getString(KEY_LUGAR);
                long hora       = terremotoItem.getLong(KEY_HORA);
                String url      = terremotoItem.getString(KEY_URL);

                earthquakes.add(new Terremoto(magnitud,lugar,hora,url));
            }
